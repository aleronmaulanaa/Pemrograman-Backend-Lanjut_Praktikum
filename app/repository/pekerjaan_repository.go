...
// func GetTrashPekerjaanRBAC() ([]model.PekerjaanAlumni, error) {
// 	rows, err := database.DB.Query(`
// 		SELECT id, alumni_id, nama_perusahaan, posisi_jabatan, bidang_industri, lokasi_kerja, gaji_range,
...
// 		if err := rows.Scan(&p.ID, &p.AlumniID, &p.NamaPerusahaan, &p.PosisiJabatan, &p.BidangIndustri, &p.LokasiKerja,
// 			&p.GajiRange, &p.CreatedAt, &p.UpdatedAt, &isDeleted); err != nil {
// 			return nil, err
// 		}
