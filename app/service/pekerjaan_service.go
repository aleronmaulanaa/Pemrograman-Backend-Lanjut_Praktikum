...
// 	if role == "admin" {
// 		// Admin → lihat semua yang dihapus
// 		rows, err = database.DB.Query(`
// 			SELECT p.id, p.alumni_id, a.user_id, a.nama AS nama_alumni,
// 				   p.nama_perusahaan, p.posisi_jabatan, p.bidang_industri, p.lokasi_kerja, p.is_deleted
// 			FROM pekerjaan_alumni p
// 			JOIN alumni a ON p.alumni_id = a.id
// 			WHERE p.is_deleted IS NOT NULL
// 			ORDER BY p.is_deleted DESC`)
// 	} else {
// 		// User → lihat hanya miliknya
// 		rows, err = database.DB.Query(`
// 			SELECT p.id, p.alumni_id, a.user_id, a.nama AS nama_alumni,
// 				   p.nama_perusahaan, p.posisi_jabatan, p.bidang_industri, p.lokasi_kerja, p.is_deleted
// 			FROM pekerjaan_alumni p
...
// 			ORDER BY p.is_deleted DESC`, int(userID))
// 	}
...
// 	var deletedList []DeletedPekerjaan
// 	for rows.Next() {
// 		var d DeletedPekerjaan
// 		err := rows.Scan(&d.ID, &d.AlumniID, &d.UserID, &d.NamaAlumni,
// 			&d.NamaPerusahaan, &d.PosisiJabatan, &d.BidangIndustri, &d.LokasiKerja, &d.IsDeleted)
// 		if err != nil {
// 			return c.Status(500).JSON(fiber.Map{"error": err.Error()})
// 		}
// 		deletedList = append(deletedList, d)
// 	}
...
// 	// Cek apakah data pekerjaan valid dan siapa pemiliknya
// 	var alumniUserID int
// 	err = database.DB.QueryRow(`
// 		SELECT a.user_id
// 		FROM pekerjaan_alumni p
// 		JOIN alumni a ON p.alumni_id = a.id
// 		WHERE p.id=$1`, idInt).Scan(&alumniUserID)
...
// 	// Lakukan restore (hapus nilai is_deleted)
// 	_, err = database.DB.Exec(`UPDATE pekerjaan_alumni SET is_deleted=NULL WHERE id=$1`, idInt)
// 	if err != nil {
// 		return c.Status(500).JSON(fiber.Map{"error": err.Error()})
// 	}
...
// // DELETE Permanen hanya jika is_deleted IS NOT NULL
...
// 	// Cek apakah pekerjaan ada dan sudah soft delete
// 	var alumniUserID int
// 	var isDeleted *time.Time
// 	err = database.DB.QueryRow(`
// 		SELECT a.user_id, p.is_deleted
// 		FROM pekerjaan_alumni p
// 		JOIN alumni a ON p.alumni_id = a.id
// 		WHERE p.id=$1`, idInt).Scan(&alumniUserID, &isDeleted)

// 	if err != nil {
// 		return c.Status(404).JSON(fiber.Map{"error": "Pekerjaan tidak ditemukan"})
// 	}

// 	// Pastikan sudah soft deleted
// 	if isDeleted == nil {
// 		return c.Status(400).JSON(fiber.Map{"error": "Data belum dihapus (soft delete) sehingga tidak bisa dihapus permanen"})
// 	}

// 	// Jika user biasa, pastikan hanya bisa hapus miliknya sendiri
// 	if role != "admin" && int(userID) != alumniUserID {
// 		return c.Status(403).JSON(fiber.Map{"error": "Forbidden: Anda hanya bisa menghapus pekerjaan Anda sendiri"})
// 	}

// 	// Lakukan hard delete
// 	_, err = database.DB.Exec(`DELETE FROM pekerjaan_alumni WHERE id=$1`, idInt)
// 	if err != nil {
// 		return c.Status(500).JSON(fiber.Map{"error": err.Error()})
// 	}
